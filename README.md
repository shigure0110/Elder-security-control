...
工作流已改为直接使用 CI 安装的 Gradle（`gradle assembleDebug`），不再依赖 wrapper 二进制文件。

这样即使仓库里没有 `gradle-wrapper.jar`，Actions 也能正常构建。


### 二进制限制说明
为避免“拉取失败/不支持二进制文件”，仓库不再跟踪 `gradle/wrapper/gradle-wrapper.jar`。CI 统一使用 `gradle assembleDebug`，不依赖 wrapper 二进制。
